...
UNIT_TEST("NetworkPayload:ReadHeader:16bitLength")
{
	...
	std::array<byte, 1024> headerDataArr;
	std::copy(headerData.begin(), headerData.end(), headerDataArr.begin());

	Range<std::array<byte, READ_BUFFER_SIZE>> range(headerDataArr);

	auto payloadInfo = ReadHeader(range);

	UnitTest::Assert(payloadInfo.commandName == CommandNames::CONNECT && payloadInfo.payloadLength == 3);
} UNIT_TEST_END
...
UNIT_TEST("NetworkPayload:ReadHeader:32bitLength")
{
	...
	UnitTest::Assert(payloadInfo.commandName == CommandNames::CONNECT && payloadInfo.payloadLength == 986246);
} UNIT_TEST_END
...
UNIT_TEST("NetworkPayload:ReadHeader:64bitLength")
{
	...
	UnitTest::Assert(payloadInfo.commandName == CommandNames::ERROR_MSG && payloadInfo.payloadLength == 5000000000);
} UNIT_TEST_END
...
UNIT_TEST("NetworkPayload:ReadHeader:8bitLength")
{
	...
	UnitTest::Assert(payloadInfo.commandName == CommandNames::CONNECT && payloadInfo.payloadLength == 86);
} UNIT_TEST_END
...
UNIT_TEST("NetworkPayload:ReadHeader:8bitLength:2")
{
	...
	UnitTest::Assert(payloadInfo.commandName == CommandNames::ERROR_MSG && payloadInfo.payloadLength == 3);
} UNIT_TEST_END