...
private:
	template <typename T_CONNECTION, typename... T_Args>
	void HandleAcceptAsync(const TCPConnection::TCPConnection_ptr tcpConnection, const boost::system::error_code& error, T_Args&&... args)
	{
		tcpConnection->HandleAcceptedAsync(error);

		StartAccept<T_CONNECTION>(args...);	// Once a connection is accepted, we call back the acceptor to handle the next connection
	}
};